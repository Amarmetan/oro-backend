...
export function initDatabase() {
  ...
  try {
    ...
    db.exec(`
      UPDATE users 
      SET is_registered = 1 
      WHERE user_id IN (7770001, 8880002, 9990003, 9990004);
    `);
    ...
    if (!unreg) {
      db.prepare(`
        INSERT INTO users (user_id, username, first_name, last_name, balance, is_vip, vip_until, points, is_admin, is_partner, is_registered, phone_number, preferred_language, created_at)
  ...
  try {
    ...
    const existing = db.prepare(`
      ...
      WHERE user_id = ? AND movie_id = ? 
        AND (purchase_type = 'lifetime' OR expires_at > datetime('now'))
...
export async function purchase_folder(
  ...
  try {
    ...
    `).run(now, userId, bundlePrice, newBuyerBalance, refId, `Purchased entire "${category}" Folder Bundle (${movies.length} films)`);
